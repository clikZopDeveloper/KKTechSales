...
class ClientsListActivity : AppCompatActivity(), ApiResponseListner,
    GoogleApiClient.OnConnectionFailedListener,
    ConnectivityListener.ConnectivityReceiverListener {
    private lateinit var binding: ActivityClientListBinding
    private lateinit var mAllAdapter: ClientListAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.igToolbar.tvTitle.text = "All Clients"
    ...
    fun apiUpdateClient(id: Int, doa: String, dob: String) {
        ...
        apiClient.progressView.showLoader()
        apiClient.getApiPostCall(ApiContants.updateClient, params)

    }

    fun handleClientList(data: List<ClientBean.Data>) {
        binding.rcAllLead.layoutManager = LinearLayoutManager(this)
        mAllAdapter = ClientListAdapter(this, data, object :
            RvPlumberListner {
    ...
    override fun success(tag: String?, jsonElement: JsonElement?) {
        try {
            ...
            if (tag == ApiContants.getClient) {
                val clientListBean = apiClient.getConvertIntoModel<ClientBean>(
                    jsonElement.toString(),
                    ClientBean::class.java
                )

                if (clientListBean.error==false) {
                    handleClientList(clientListBean.data)
                }
            }
            ...
            if (tag == ApiContants.updateClient) {
