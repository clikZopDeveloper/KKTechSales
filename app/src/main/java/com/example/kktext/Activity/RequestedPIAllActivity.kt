...
import com.example.kktext.Model.AllRequestedPIBean
...
class RequestedPIAllActivity : AppCompatActivity(), ApiResponseListner,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.igToolbar.tvTitle.text = "All Requested PI"
    ...
    fun apiGetRequestedPI() {
        SalesApp.isAddAccessToken = true
        apiClient = ApiController(this, this)
        val params = Utility.getParmMap()
        apiClient.progressView.showLoader()
        apiClient.getApiPostCall(ApiContants.GetRequestedPI, params)
    }

    fun handleAllRequestPI(data: List<AllRequestedPIBean.Data>) {
        binding.rcAllLead.layoutManager = LinearLayoutManager(this)
        var mAdapter = AllRequestPIAdapter(this, data, intent.getStringExtra("leadStatus"), object :
    ...
    override fun success(tag: String?, jsonElement: JsonElement?) {
        try {
            ...
            if (tag == ApiContants.GetRequestedPI) {
                val allRequestedPIBean = apiClient.getConvertIntoModel<AllRequestedPIBean>(
                    jsonElement.toString(),
                    AllRequestedPIBean::class.java
                )
                //   Toast.makeText(this, allStatusBean.msg, Toast.LENGTH_SHORT).show()
                if (allRequestedPIBean.error==false) {
                    handleAllRequestPI(allRequestedPIBean.data)
                }
            }
